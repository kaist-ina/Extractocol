...
import soot.jimple.infoflow.BackwardInfoflowForHeap;
import soot.jimple.infoflow.Infoflow;
import soot.jimple.infoflow.PairingInfoflow;
...
public class SetupApplication
{
	...
	private boolean backwardSlice = false;
	 ...
	 * 
	 * @param androidJar
	 *            The path to the Android SDK's "platforms" directory if Soot shall automatically select the JAR file to be used or the path to a single JAR file to force one.
	...
	public SetupApplication(String androidJar, String apkFileLocation, String additionalClasspath, IIPCManager ipcManager)
	{
		...
		this.apkFileLocation = Constants.serializationDirName + File.separator + apkFileLocation;
	...
	/**
	 * Sets the class names of callbacks. If this value is null, it automatically loads the names from AndroidCallbacks.txt as the default behavior.
	 * 
	...
	/**
	 * Sets the taint wrapper to be used for propagating taints over unknown (library) callees. If this value is null, no taint wrapping is used.
	 * 
	 * @param taintWrapper
	...
	/**
	 * Gets the taint wrapper to be used for propagating taints over unknown (library) callees. If this value is null, no taint wrapping is used.
	 * 
	...
	public void calculateSourcesSinksEntrypoints(ISourceSinkDefinitionProvider sourcesAndSinks) throws IOException, XmlPullParserException
	{
		...
		// Create the SourceSinkManager
		{
			...
			sourceSinkManager = new AccessPathBasedSourceSinkManager(this.sourceSinkProvider.getSources(), this.sourceSinkProvider.getSinks(), callbacks, layoutMatchingMode,
	...
	/**
	 * Gets the source/sink manager constructed for FlowDroid. Make sure to call calculateSourcesSinksEntryPoints() first, or you will get a null result.
	 * 
	...
	private void initializeSoot()
	{
		...
		Options.v().set_output_dir("C:\\Users\\appff\\Desktop\\ted_jimple");
		if (forceAndroidJar)
	...
	public InfoflowResults runInfoflow(ResultsAvailableHandler onResultsAvailable)
	{
		...
		System.out.println("Running data flow analysis on " + apkFileLocation + " with " + getSources().size() + " sources and " + getSinks().size() + " sinks...");
		// JM backward taint analysis
		if (backwardSlice)
		{
			// File f = new File("Pairing.json");
			// if (f.delete())
			// System.out.println("\"Pairing.json\" is deleted.");
			// else
			// System.err.println("Cannot delete \"Pairing.json\"");c
			BackwardInfoflow backinfo;
			if (cfgFactory == null)
				backinfo = new BackwardInfoflow(getClasspath(), forceAndroidJar, null, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
			else
				backinfo = new BackwardInfoflow(getClasspath(), forceAndroidJar, cfgFactory, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
			final String path;
			if (forceAndroidJar)
				path = androidJar;
			else
				path = Scene.v().getAndroidJarPath(androidJar, apkFileLocation);
			backinfo.setTaintWrapper(taintWrapper);
			if (onResultsAvailable != null)
				backinfo.addResultsAvailableHandler(onResultsAvailable);
			System.out.println("Starting infoflow computation...");
			backinfo.setSootConfig(sootConfig);
			backinfo.setStopAfterFirstFlow(stopAfterFirstFlow);
			backinfo.setEnableImplicitFlows(enableImplicitFlows);
			backinfo.setEnableStaticFieldTracking(enableStaticFields);
			backinfo.setEnableExceptionTracking(enableExceptions);
			BackwardInfoflow.setAccessPathLength(accessPathLength);
			backinfo.setFlowSensitiveAliasing(flowSensitiveAliasing);
			backinfo.setIgnoreFlowsInSystemPackages(ignoreFlowsInSystemPackages);
			backinfo.setCodeEliminationMode(codeEliminationMode);
			backinfo.setInspectSources(false);
			backinfo.setInspectSinks(false);
			backinfo.setCallgraphAlgorithm(callgraphAlgorithm);
			if (null != ipcManager)
			{
				backinfo.setIPCManager(ipcManager);
			}
			backinfo.computeInfoflow(apkFileLocation, path, entryPointCreator, sourceSinkManager);
			this.maxMemoryConsumption = backinfo.getMaxMemoryConsumption();
			return backinfo.getResults();
		}
		else
			if (Constants.heapobject)
			{
				System.out.println("Start Heap Finding Module");
				BackwardInfoflowForHeap backinfo;
				if (cfgFactory == null)
					backinfo = new BackwardInfoflowForHeap(getClasspath(), forceAndroidJar, null, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
				else
					backinfo = new BackwardInfoflowForHeap(getClasspath(), forceAndroidJar, cfgFactory, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
				final String path;
				if (forceAndroidJar)
					path = androidJar;
				else
			...
			// JM taint analysis for Pairing
			else
				if (pairing)
				{
					PairingInfoflow info;
					if (cfgFactory == null)
						info = new PairingInfoflow(getClasspath(), forceAndroidJar, null, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
					else
						info = new PairingInfoflow(getClasspath(), forceAndroidJar, cfgFactory, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
					final String path;
					if (forceAndroidJar)
						path = androidJar;
					else
				...
				// JM forward taint analysis
				else
				{
					Infoflow info;
					if (cfgFactory == null)
						info = new Infoflow(getClasspath(), forceAndroidJar, null, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
					else
						info = new Infoflow(getClasspath(), forceAndroidJar, cfgFactory, new DefaultPathBuilderFactory(pathBuilder, computeResultPaths));
					final String path;
					if (forceAndroidJar)
						path = androidJar;
					else
	...
	public AndroidEntryPointCreator createEntryPointCreator(Set<String> Entries)
	{
		AndroidEntryPointCreator entryPointCreator = new AndroidEntryPointCreator(new ArrayList<String>(Entries));
		Map<String, List<String>> callbackMethodSigs = new HashMap<String, List<String>>();
		for (String className : this.callbackMethods.keySet())
		{
			List<String> methodSigs = new ArrayList<String>();
	...
	/**
	 * Gets the entry point creator used for generating the dummy main method emulating the Android lifecycle and the callbacks. Make sure to call calculateSourcesSinksEntryPoints() first, or you will get a null result.
	 * 
	...
	/**
	 * Sets whether implicit flow tracking shall be enabled. While this allows control flow-based leaks to be found, it can severly affect performance and lead to an increased number of false positives.
	 * 
	 ...
	 * 
	 * @return The extra Soot configuration options to be used when running the analysis, null if the defaults shall be used
	 */
	 ...
	 * 
	 * @param config
	 *            The extra Soot configuration options to be used when running the analysis, null if the defaults shall be used
	 */
	...
	public void setBackwardSlice(boolean backward)
	{
		this.backwardSlice = backward;
	}
