...
public class CallGraph implements Iterable<Edge>
{
	...
	public boolean removeAllEdgesOutOf(Unit u)
	{
		...
		for (QueueReader<Edge> edgeRdr = listener(); edgeRdr.hasNext();)
		{
			...
			if (e.srcUnit() == u)
			{
//				System.out.println("Removing Edge : " + u);
				removeEdge(e);
				hasRemoved = true;
			}
		}
	...
	/**
	 * Swaps an invocation statement. All edges that previously went from the given statement to
	 * some callee now go from the new statement to the same callee. This method is intended to be
	 * used when a Jimple statement is replaced, but the replacement does not semantically affect
	 * the edges.
	 * 
	 * @param out
	 *            The old statement
	...
	/**
	 * Removes the edge e from the call graph. Returns true iff the edge was originally present in
	 * the call graph.
	 */
