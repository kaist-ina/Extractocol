		...
		} else
		{
			...
			if (appPath != null)
			{
				...
				for (int i = 2; i < 11; i++)
				{
					...
					if (new File(path2 + Constants.getMultiDexName(path2 + "/SerializationFiles", i)).exists())
...
//		reconHttpTransacs.makeSourcesAndSinks();
		...
		if (Constants.Serializing)
		{
			try
			{
				...
				CFGs.Serialize(ICFG_CASE.FORWARD);
				CFGs.SerializeEPC(ICFG_CASE.FORWARD);
				System.out.println("Finished Serialization");
				
				System.out.println("write shimple start....");
				for (SootClass sc : Scene.v().getClasses())
				{
					WriteSootClass(sc);
				}
				System.out.println("Finish write shimple");
			}catch (IOException e)
			{
				e.printStackTrace();
			}
