...
import Extractocol.BufferExtractor_Request.Helper.CFGSerializer.ICFG_CASE;
import Extractocol.Common.APK.UnzipAPK;
import Extractocol.Common.Pairing.BuildPairJson;
import Extractocol.Common.Pairing.PairingHandler;
import Extractocol.Tester.BackendFunctions.BackendTester_Request;
	...
	private void initializeSoot(String appPath, String libPath, Collection<String> classes, String extraSeed)
	{
		...
		else
		{
			...
			if (appPath != null)
			{
				...
				try
				{
					UnzipAPK.UnzipAPK(appPath, path2);
				}
				catch (Exception e)
				{
				...
				for (int i = 2; i < 11; i++)
				{
					...
					if (new File(path2 + "/SerializationFiles"+ Constants.getMultiDexName(path2 + "/SerializationFiles", i)).exists()){
						System.out.println("Read dex file: " + path2 + "/SerializationFiles"+ Constants.getMultiDexName(path2 + "/SerializationFiles", i));
						processDirs.add(path2 + "/SerializationFiles"+ Constants.getMultiDexName(path2 + "/SerializationFiles", i));
					}
	...
	private void runAnalysis(final ISourceSinkManager sourcesSinks, final Set<String> additionalSeeds)
	{
		...
		if (additionalSeeds != null)
			for (String meth : additionalSeeds)
			{
				...
				Abstraction zeroAbs = backwardProblem.zeroValue();
				LinkedList<String> taintm = new LinkedList<String>();
				taintm.add(m.getSignature());
				zeroAbs.settaintmethods(taintm);
				backwardProblem.addInitialSeeds(m.getActiveBody().getUnits().getFirst(), Collections.singleton(zeroAbs));
			}
		...
		/*clear DPs for CodeInject*/
		try {
			File CodeInjectDir = new File(Constants.CodeInjectPath());
			
			if (!CodeInjectDir.exists())
			{
				CodeInjectDir.mkdir();
			}
			PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(new File(Constants.CodeInjectPath() + "/" + Constants.CodeInjectDPsName()), false)));
			pw.close();
		...
		// JM new taint module
		if (Constants.flowsensitive)
		{
			List<FlowsensitiveSlice> FlowsensitiveSlices = backwardProblem.getBackwardResults();
//			printEPs(FlowsensitiveSlices);
			for (FlowsensitiveSlice fs : FlowsensitiveSlices)
			{
				//if(!fs.getEP().contains("requestService"))
					//continue;
				
				EPcontainer epcontainer = new EPcontainer();
				System.out.println("\n(" + EPCount + "/" + FlowsensitiveSlices.size() + ") Printing..");
				System.out.println("+ DP Stmt : " + fs.getDPStmt());
				System.out.println("+ DP SootMethod : " + fs.getDPMethod());
				
				/*save DPs for CodeInject*/
				PrintWriter pw = null;
				try {
					pw = new PrintWriter(new BufferedWriter(new FileWriter(new File(Constants.CodeInjectPath() + "/" + Constants.CodeInjectDPsName()), false)));
				    pw.println(fs.getDPStmt());
				    pw.println(fs.getDPMethod());
				}catch (IOException e) {
				    e.printStackTrace();
				}finally{
				    if(pw != null){
				...
				//Useless dpoint and EntrypointFinder
				epcontainer.setDP(fs.getDPMethod());
				epcontainer.setDPStmt(fs.getDPStmt());
				epcontainer.DemarcationInvoke = fs.getDPStmt().getInvokeExpr().getMethod().getSubSignature();
				
				//In this control block, EP is one. EP is divided by each call flow.
				List<String> Epointlist = new ArrayList<String>();
				Epointlist.add(fs.getEP());
				
				//We need to modify this method.
//				Epointlist = CheckConstantParamEpoint(Epointlist);
				EPCount++;
				
				List<SootMethod> FinalEps = new ArrayList<SootMethod> ();
				
				System.out.println("+ EntryPoint : " + fs.getEP());
				...
				//System.out.println("\t++ " + fs.getDPStmt().getInvokeExpr().getMethod());
				
				Set<SootMethod> taintSet = new HashSet<SootMethod>();
				taintSet.add(fs.getDPStmt().getInvokeExpr().getMethod());
				for (String sm : fs.getTaintmethods())
				{
					if (Scene.v().getMethod(sm) == null)
						System.out.println("null taintsm");
					else
						taintSet.add(Scene.v().getMethod(sm));
				}
				
				for (SootMethod sm : taintSet)
				{
					System.out.println("\t++ " + sm.getSignature());
				}
				
				epcontainer.setMethodSet(taintSet);
				epcontainer.setCallflow(null);
		...
		else
		{
			for (AbstractSlice as : ass)
			{
				...
				PrintWriter pw = null;
				try {
					pw = new PrintWriter(new BufferedWriter(new FileWriter(new File(Constants.CodeInjectPath() + "/" + Constants.CodeInjectDPsName()), true)));
					System.out.println("hhyeo");
				    pw.println(as.getDpStmt());
				    pw.println(as.getDpMethod());
				}catch (IOException e) {
				    e.printStackTrace();
				}finally{
				    if(pw != null){
				...
				// print
				// When a method's caller has only constant variables, taint module
				// stops analysis.
				// To building accurate signatures, Extractocol should set the
				// caller method as a EP.
				Epointlist = CheckConstantParamEpoint(Epointlist);
				for (String epoint : Epointlist)
				{
				...
				taintSet.add(as.getDpStmt().getInvokeExpr().getMethod());
				...
				if (Constants.removeWrongEp)
					RemoveWrongEp(dPoints, Epointlist, as, epcontainer, taintSet, FinalCf);
				else
					noRemoveWrongEp(dPoints, Epointlist, as, epcontainer, taintSet, FinalCf);
		...
		if (Constants.Serializing)
		{
			try
			{
				...
				CFGs.Serialize(ICFG_CASE.BACKWARD);
				CFGs.SerializeEPC(ICFG_CASE.BACKWARD);
	...
	private List<String> CheckConstantParamEpoint(List<String> epointlist)
	{
		...
		for (String epoint : epointlist)
		{
			boolean isAllWithConstants = true;
			
			Collection<Unit> callers = iCfg.getCallersOf(Scene.v().getMethod(epoint));
			for (Unit caller : callers)
			{
				InvokeExpr ie;
				
				if(caller instanceof AssignStmt)
					ie = ((AssignStmt)caller).getInvokeExpr();
				else
					ie = ((InvokeStmt) caller).getInvokeExpr();
				...
				for(Value arg : ie.getArgs()){
					...
					else
					{
						...
						isAllWithConstants = false;
						break;
					}
				}
				
				if(isAllConstants)
					result.add(iCfg.getMethodOf(caller).getSignature());
			}
			
			if(!isAllWithConstants)
				result.add(epoint);
		}
		...
		if (m.hasActiveBody())
		{
			...
			for (Unit u : units)
			{
				...
				if (sourcesSinks.getSourceInfo(s, iCfg) != null)
				{
					...
					// {
					// JM assign taintmethods for init seed.
					Abstraction zeroAbs = backwardProblem.zeroValue();
					LinkedList<String> taintm = new LinkedList<String>();
					taintm.add(m.getSignature());
					zeroAbs.settaintmethods(taintm);
					backwardProblem.addInitialSeeds(u, Collections.singleton(zeroAbs));
	...
	private void printEPs(List<FlowsensitiveSlice> FlowsensitiveSlices)
	{
		Set<String> unique = new HashSet<String>();
		List<LinkedList<String>> taintmethods = new ArrayList<LinkedList<String>>();
		for (FlowsensitiveSlice fs : FlowsensitiveSlices)
		{
			if (fs.getEP().contains("requestService"))
			{
				if (unique.add(fs.getEP()))
				{
					taintmethods.add(fs.getTaintmethods());
				}
			}
		}
		int z = 0;
		for (String a : unique)
		{
			System.out.println("EP[" + z + "] : " + a);
			System.out.println("Taintmethods : " + taintmethods.get(z++));
		}
	}
}
