...
public class AssignStmtHandler extends AbstractUnitHandler
{
	...
	private static void IncludedInvokeExprProcessor(AssignStmt as, ParameterBucket pb, SignatureBuilder_Request sb, Unit ut)
	{
		...
		// If invoke handler fails, make temp bfn -- woohyun (20160330)
		// TODO: LeftOp should be updated even though there is (1) no taint method or (2) no semantic model. -- Byungkwon (20161116)
		if (pb.blockBFTtable.get(as.getLeftOp().toString()) == null || pb.blockBFTtable.get(as.getLeftOp().toString()).isEmpty())
		{
			ArrayList<BFNode> arraylist = new ArrayList<BFNode>();
			BFNode bfn = new BFNode();
			bfn.makeUrlBfn(".*");
			arraylist.add(bfn);
			pb.blockBFTtable.put(as.getLeftOp().toString(), arraylist);
		}
	}
	...
	public void HandleUnit(Unit ut, ParameterBucket pb, SignatureBuilder_Request sb)
	{
		...
		sb.AddContextTable(pb.CurrentSM, as);

		if (!sb.hasNode(pb.blockBFTtable, pb.strDest))
			pb.blockBFTtable.put(pb.strDest, new ArrayList<BFNode>());

		if (as.getRightOp() instanceof PhiExpr)
			PhiExprProcessor(as, pb, sb);
		else if (as.containsInvokeExpr())
			IncludedInvokeExprProcessor(as, pb, sb, ut);
		else{
			// BK keep type name 
			if (as.getRightOp() instanceof NewExpr)
				pb.NewAssignmentHandler(as);
			
			NormalAssignStmtProcessor(as, pb, sb);
		}
		
		// TODO: reflect variable's value in BFTTable to varTable when a semantic model is applied.
		if(Constants.isSemantic){
			UrlBuilder ub = new UrlBuilder();
			String res = ub.GenRegex(null, pb.blockBFTtable, pb.strDest);
			
			// case 1: $r = (semantic)
			if(ValueEntryTracking.isLeftOpVariable(as))
				pb.varTable.addConstantValue(pb.strDest, res);
			
			// case 2: <> = (semantic) (There might be no case like this.)
			if(ValueEntryTracking.isLeftOpHeap(as))
				Constants.reqRespInfo.addConstantEntry(ValueEntryTracking.getLeftHeap(as), res);
		}
		...
		ValueEntryTracking.heapAndVarTracking(as, pb, true);
