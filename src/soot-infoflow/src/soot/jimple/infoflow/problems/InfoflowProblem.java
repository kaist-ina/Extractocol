...
public class InfoflowProblem extends AbstractInfoflowProblem {
	...
	private final boolean DEBUG_CALL = false;
	private final boolean DEBUG_RETURN = false;
	private final boolean DEBUG_CALLTORETURN = false;
	
	// forward slicing only: for some reason, AbstractSlice class is not compatible with forward slicing,
	// so we create additional data structure that needs for forward slicing. 
	private HashMap<Stmt, HashSet<SootMethod>> slicedMethodsForwardOnly = new HashMap<Stmt, HashSet<SootMethod>>();
	...
	// forward slicing only: for some reason, AbstractSlice class is not compatible with forward slicing,
	// so we create additional data structure that needs for forward slicing. 
	private void addForwardSlicedMethod(Stmt dpStmt, SootMethod slicedMethod) {
		if (slicedMethodsForwardOnly.containsKey(dpStmt)) {
			slicedMethodsForwardOnly.get(dpStmt).add(slicedMethod);
		}
		else {
			HashSet<SootMethod> methods = new HashSet<SootMethod>();
			methods.add(slicedMethod);
			slicedMethodsForwardOnly.put(dpStmt, methods);
		}
	}
	
	public HashMap<Stmt, HashSet<SootMethod>> getForwardSlicedMethods() {
		return slicedMethodsForwardOnly;
	}
	
	public HashMap<Stmt, List<SootMethod>> getForwardEPs() {
		HashMap<Stmt, List<SootMethod>> stmtAndeps = new HashMap<Stmt, List<SootMethod>>();
		
		for (Stmt dpStmt : slicedMethodsForwardOnly.keySet()) {
			List<SootMethod> eps = new ArrayList<SootMethod>();
			HashSet<SootMethod> methods = slicedMethodsForwardOnly.get(dpStmt);
			for (SootMethod method : methods) {
				Collection<Unit> callerStmts = interproceduralCFG().getCallersOf(method);
				for (Unit callerStmt : callerStmts) {
					if (!methods.contains(interproceduralCFG().getMethodOf(callerStmt))) {
						eps.add(method);
					}
				}
			}
			
			stmtAndeps.put(dpStmt, eps);
		}
		
		return stmtAndeps;
	}
	
	public HashSet<SootMethod> getForwardEPs(Stmt dpStmt) {
		HashSet<SootMethod> eps = new HashSet<SootMethod>();
		HashSet<SootMethod> methods = slicedMethodsForwardOnly.get(dpStmt);
		for (SootMethod method : methods) {
			Collection<Unit> callerStmts = interproceduralCFG().getCallersOf(method);
			for (Unit callerStmt : callerStmts) {
				if (!methods.contains(interproceduralCFG().getMethodOf(callerStmt))
						|| interproceduralCFG().getMethodOf(callerStmt).getName().equals("dummyMainMethod")) {
					eps.add(method);
				}
			}
		}
		...
		return new FlowFunctions<Unit, Abstraction, SootMethod>() {
			...
			private Set<Abstraction> notifyOutFlowHandlers(Unit stmt,
				...
				if (!outgoing.isEmpty()) {
					for (Abstraction abs : outgoing) {
						boolean found = false;
						if (abs != null && abs.getSourceContext() != null && abs.getCurrentStmt() != null) {
							for (AbstractSlice as : ass) {
								if (as.hasDemarcationPoint(
										abs.getSourceContext().getStmt(),
										interproceduralCFG().getMethodOf(abs.getSourceContext().getStmt()))) {
									as.addSlice(abs.getCurrentStmt(),
											interproceduralCFG().getMethodOf(abs.getCurrentStmt()));
									found = true;
									
									// for forward only
									addForwardSlicedMethod(abs.getSourceContext().getStmt(),
							...
							if (!found) {
								AbstractSlice newas = new AbstractSlice(
										abs.getSourceContext().getStmt(), 
										interproceduralCFG().getMethodOf(abs.getSourceContext().getStmt()), 
										abs.getCurrentStmt(),
										interproceduralCFG().getMethodOf(abs.getCurrentStmt()));
					...
					private Set<Abstraction> computeTargetsInternal(Abstraction source,
						...
						if (DEBUG_RETURN) {
							System.out.println("\n[*] ReturnFlow" + "\ncallSite : " 
									+ callSite + "\ncallee : " + callee + "\nexitStmt : " + exitStmt
									+ "\nretSite : " + retSite + "\nsource : " + source +"\nmethod: " 
									+ interproceduralCFG().getMethodOf(callSite));
						}
			...
			public FlowFunction<Abstraction> getCallToReturnFlowFunction(final Unit call, final Unit returnSite) {
				...
				if (!(call instanceof Stmt)
						|| interproceduralCFG().getMethodOf(returnSite).getName().equals("dummyMainMethod"))
					return KillAll.v();
	 ...
	 */
	public List<AbstractSlice> getSliceResults() {
		return ass;
	}

	public List<AbstractSlice> getResultsOfslices() {
		return this.ass;
	}
}