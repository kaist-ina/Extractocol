...
public class MySetupApplication {
	...
	/**
	 * Creates a new instance of the {@link MySetupApplication} class
	 * 
	 * @param androidJar
	...
	public void calculateSourcesSinksEntrypoints(ISourceSinkDefinitionProvider sourcesAndSinks)
		...
		//soot.G.reset(); // Commented by BK (I don't think it is required.)
	...
	public InfoflowResults runInfoflow(ResultsAvailableHandler onResultsAvailable) {
		...
		MyBackwardInfoflow info;

		if (cfgFactory == null)
			info = new MyBackwardInfoflow(androidJar, forceAndroidJar, null,
					new DefaultPathBuilderFactory(config.getPathBuilder(),
							config.getComputeResultPaths()));
		else
			info = new MyBackwardInfoflow(androidJar, forceAndroidJar, cfgFactory,
					new DefaultPathBuilderFactory(config.getPathBuilder(),
							config.getComputeResultPaths()));
		...
		info.setSourceSinkProvider(sourceSinkProvider); /** Added by BK */
		//myFunc(path, info);
	...
	// Added by BK
	private void myFunc(String path, MyBackwardInfoflow info){
		soot.G.reset();
		Options.v().set_src_prec(Options.src_prec_apk);
		//Options.v().set_process_dir();
		Options.v().set_soot_classpath(apkFileLocation + ";" + path);
		Options.v().set_android_jars(androidJar);
		...
		SootMethod entryPoint = this.getEntryPointCreator().createDummyMain();
		...
		/*DefaultBiDiICFGFactory myCFGfactory = new DefaultBiDiICFGFactory();
		myCFGfactory.setIsAndroid(true);
		
		IInfoflowCFG icfg = myCFGfactory.buildBiDirICFG(config.getCallgraphAlgorithm(),
        		config.getEnableExceptionTracking());
		
		for (SootClass sc : Scene.v().getClasses()) {
			for (SootMethod sm : sc.getMethods()) {
				if(sm.toString().contains("com.contextlogic.wish.http.HttpRequest: void run()"))
				{
					Collection<Unit> callsites = icfg.getCallersOf(sm);
					System.out.println("The # of callSites is " + callsites.size());
				}
				if(sm.toString().contains("<com.contextlogic.wish.api.core.WishApi: com.contextlogic.wish.api.core.WishApiRequest apiRequest(java.lang.String,com.contextlogic.wish.http.HttpRequestParams,com.contextlogic.wish.api.core.WishApiCallback)>"))
				{
					Collection<Unit> callsites = icfg.getCallersOf(sm);
					System.out.println("The # callSites of apiRequest(#3) is " + callsites.size());
				}
				
				if(sm.toString().contains("<com.contextlogic.wish.api.core.WishApi: com.contextlogic.wish.api.core.WishApiRequest apiRequest(java.lang.String,com.contextlogic.wish.http.HttpRequestParams,boolean,com.contextlogic.wish.api.core.WishApiCallback)>"))
				{
					Collection<Unit> callsites = icfg.getCallersOf(sm);
					System.out.println("The # callSites of apiRequest(#3) is " + callsites.size());
				}
				...
				if (sm.hasActiveBody()) {
					if(!sm.toString().contains("<com.contextlogic.wish.http.WishHttpClient: void request(com.contextlogic.wish.http.WishHttpClient$RequestPool,java.lang.Object,java.lang.String,com.contextlogic.wish.http.HttpRequest$RequestType,com.contextlogic.wish.http.HttpRequestParams,com.contextlogic.wish.http.HttpResponseHandler)>"))
						continue;
					
					for (Unit ut : sm.getActiveBody().getUnits()) {
						if(ut.toString().contains("<java.util.concurrent.ThreadPoolExecutor: java.util.concurrent.Future submit(java.lang.Runnable)>")){
							Collection<SootMethod> callees = icfg.getCalleesOfCallAt(ut);
							System.out.println("The # of callees is " + callees.size());
						}
					}
				}
			}
