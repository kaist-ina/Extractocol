...
public class SerializableCFG
{
	...
	{
		ExtractocolLogger.Log("Processing callees And callers");
		ProcessingCalleeAndCallers();
		ExtractocolLogger.Log("Processing super/sub tables");
	...
	/*public void SetTaintFunctions()
	{
		...
		for (EPcontainer epc : epcontainerlist)
		{
			...
			// If flowsensitive flag is true, taintmethodset is identified by index.
	...
	public ArrayList<String> GetTaintFunctions(String DemarcationPoint, String string, int _index)
	{
		int i = 0;
	...
	private void ProcessingCalleeAndCallers()
	{
		for (SootClass sc : Scene.v().getClasses())
		{
			if(sc.getName().equals("dummyMainClass")) // Added by BK
				continue; 
			...
			for (SootMethod sm : sc.getMethods())
			{
				...
				if (callers.size() > 0)
				{
					for (Unit ut : callers)
					{
						...
						{
							ExtractocolLogger.Warn("WARNING [Method of CallSite]: " + target  + ", [CallSite]: " + ut + ", [Callee]: " + sm.toString());
	...
	{
		SootMethod newMethod = new SootMethod(oldMethod.getName(), oldMethod.getParameterTypes(), oldMethod.getReturnType(), oldMethod.getModifiers(), oldMethod.getExceptions());
	...
	public SootMethod getMethodOf(String MSig)
	{
		...
		else
		{
			...
			sc = Scene.v().getSootClass(JimpleName);
			if (sc == null)
				return null;
