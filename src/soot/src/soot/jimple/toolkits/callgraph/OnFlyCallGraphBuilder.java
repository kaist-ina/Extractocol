...
public final class OnFlyCallGraphBuilder
{
	...
	public void processReachables() {
		while(true) {
			...
			//System.out.println("[BK debug] Processed method: " + m.toString()); // will be removed by BK
		}
	}
				...
				if (ie instanceof InstanceInvokeExpr) {
					...
					if( subSig == sigStart ) {
						addVirtualCallSite( s, m, receiver, iie, sigRun,
							...
							|| subSig == sigFutureSubmit ) { // Augmented by BK
	...
	/** Jm missing Edge finding methods */
	public void processDPCandidates() {
		// JM start ICFG Module
		if (!soot.jimple.toolkits.callgraph.AddReachableMethods.isCalled) {
			System.out.println("[Extractocol] : connect callback");
			ConnectCallbackForEachInterfaceInvoke();
			ConnectMissingInvoke();
			soot.jimple.toolkits.callgraph.AddReachableMethods.isCalled = true;
		}
	}

	private void InvokeStmtHandler(Unit ut, SootMethod sm){
		InvokeStmt is = (InvokeStmt) ut;
		if (is.getInvokeExpr() instanceof StaticInvokeExpr || is.getInvokeExpr() instanceof VirtualInvokeExpr || is.getInvokeExpr() instanceof SpecialInvokeExpr) {
			InvokeExpr sie = (InvokeExpr) is.getInvokeExpr();
			if (Scene.v().getMethod(sie.getMethod().getSignature()) != null) {
				Edge nEdge = new Edge(sm, ut, Scene.v().getMethod(sie.getMethod().getSignature()),
						is.getInvokeExpr() instanceof StaticInvokeExpr ? Kind.STATIC : is.getInvokeExpr() instanceof VirtualInvokeExpr ? Kind.VIRTUAL : Kind.SPECIAL);
			...
			if (sie.getMethodRef().getSignature()
				...
				try {
					...
					// [Extractocol] this is occurred when target
			...
			for (SootMethod sm : sc.getMethods()) {
				if (sm.hasActiveBody()) {
					for (Unit ut : sm.getActiveBody().getUnits()) {
						...
						InvokeExpr ie = getInvokeExpr(ut); // Added by BK
						if(ie == null) continue; // Added by BK
						if(!Scene.v().containsMethod(ie.getMethod().getSignature())) continue; // Added by BK
						
						if (ut instanceof InvokeStmt) {
							InvokeStmtHandler(ut, sm);
						} else if (ut instanceof AssignStmt) {
							AssignStmtHandler(ut, sm);
						}
					}
				}
			}
	...
	/** Get invoke expression from ut
	 * 
	 * @author Byungkwon Choi
	 * @param ut unit
	 * @return InvokeExpr of ut
	 */
	private InvokeExpr getInvokeExpr(Unit ut){
		if(ut instanceof InvokeStmt){
			InvokeStmt is = (InvokeStmt) ut;
			return is.getInvokeExpr();
		}else if(ut instanceof AssignStmt){
			AssignStmt as = (AssignStmt) ut;
			if(!as.containsInvokeExpr())
				return null;
			return as.getInvokeExpr();
		}
		return null;
	}

	private void AssignStmtHandler(Unit ut, SootMethod sm) {
		AssignStmt as = (AssignStmt) ut;
		if (as.containsInvokeExpr()) {
			if (as.getInvokeExpr() instanceof StaticInvokeExpr || as.getInvokeExpr() instanceof VirtualInvokeExpr || as.getInvokeExpr() instanceof SpecialInvokeExpr) {
				...
				if (sie.getMethodRef().getSignature().equals(
					...
					if (!vie.getBase().getType().Extype.isEmpty()) {
						...
						.equals("<rx.Observable: rx.Observable create(rx.Observable$OnSubscribe)>")) {
										...
										/*
										 * $r4 = staticinvoke <rx.Observable: rx.Observable create(rx.Observable$OnSubscribe)>($r3); 
										 * $r5 = staticinvoke <rx.schedulers.Schedulers: rx.Scheduler io()>(); 
										 * $r4 = virtualinvoke $r4.<rx.Observable: rx.Observable subscribeOn(rx.Scheduler)>($r5);
										 */
	...
	private void ConnectCallbackForEachInterfaceInvoke() {
		...
		for (SootClass sc : Scene.v().getClasses()) {
			// [Extractocol] This's condition for some interfaceinvoke in specific classes.
	...
	private void ConnectInterfaceExprEdge(InterfaceInvokeExpr Iie, SootMethod sm, Unit ut)
	{
		...
		if(BaseClass.isPhantom()) // Added by BK
			return;
		
		if (BaseClass.isInterface() && !CallbackCandidateFinder.isClassInSystemPackage(Iie.getMethod().getDeclaringClass().toString())) {
			List<SootClass> subclasses;

			try {
				subclasses = Scene.v().getActiveHierarchy().getImplementersOf(BaseClass);
				// System.out.println("[OK] " + baseClassname + " in " +
				// Iie.toString());//Added by BK
				...
				//System.out.println("[FAIL] " + baseClassname + " in " + Iie.toString());//Added by BK
				e.printStackTrace();
				System.exit(0);
			}
		}
	...
	public SootClass HasClass(String ClassName)
	{
		...
		if (!CallbackCandidateFinder.isClassInSystemPackage(ClassName))
		{
			...
			for (final Unit u : smQueue.poll().getActiveBody().getUnits())
			{
				...
				if (s.containsInvokeExpr() && !cicg.edgesOutOf(u).hasNext())
				{
					...
					} else if (ie instanceof SpecialInvokeExpr)
					{
						...
						soot.jimple.toolkits.callgraph.AddReachableMethods.cg = cicg;
	...
	// To generate virtualEdge between submit() to run() (by BK)
	protected final NumberedString sigFutureSubmit = Scene.v().getSubSigNumberer().
			findOrAdd( "java.util.concurrent.Future submit(java.lang.Runnable)" ); // Added by BK
}
